...
public class Courses implements CourseInterface {
	...
	public int[] prefernces = { 0, 0, 0, 0, 0 };
	...
	public LinkedHashMap<String, Courses> readCourses() {
		return ObjectPool.getInstance().getAllCourses();
	}
	 ...
	 */
	public static Courses getNextAvailableCourse(List<Courses> allocated) {
		int numSeatAvail = 0;
		int rating = 80;
		Courses courseToRet = null;

		for (Courses course : ObjectPool.getInstance().getAllCourses().values()) {
			...
			if (60 - course.student.size() > numSeatAvail && maxRate < rating) {
				numSeatAvail = 60 - course.student.size();
				courseToRet = course;
				rating = maxRate;
			}
		}
